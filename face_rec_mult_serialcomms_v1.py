# face_rec_mult_serialcomms_v1.py
...
# if face is recognized, send serial comms to ESP32
...
serial_out_port = '/dev/cu.usbserial-1430'
ser = serial.Serial(serial_out_port, 115200, timeout=0.050)
...
# setup camera for live capture
cam = cv2.VideoCapture(0)

while cam.isOpened():
    load_time = time.time()
    # our unknown photo, may contain single or multiple faces
    success, unknown_image = cam.read()
    ...
    for face_location in face_locations_unknown_image:
        ...
        unknown_face = face_recognition.face_encodings(unknown_face_image)
        if unknown_face:
        ...
        # detect if Jon Snow
        if face_label == "Jon Snow":
            print("Jon Snow detected")
            # send data to ESP32
            ser.write('{}\n'.format(face_label).encode())
        else:
            ser.write('Unknown\n'.encode())
